...
if __name__ == "__main__":
    ...
    print("\n point X=",point.xy[0])
    print("\n point y=",point.xy[1])
    print("\n Size of Co ordinate",point._curve.modulus_bits,"bits")
    print("\n Communication overhead = ",point.xy[0].size_in_bytes()+point.xy[1].size_in_bytes(),"bytes")
    ...
    while i<=num :
    	start = time.time()
   	pdbl=EccPoint.double(point)
    	end = time.time()
  	elapsed=(end-start)*1000
	sum+=elapsed
    	#print("\n Point Double X= ",pdbl.xy[0],"\n and Y Co ordinate = ",pdbl.xy[1] )
    	#print("\n Doubling Time = ",elapsed, "ms")
	i+=1
    average=sum/num
    print("\n Average Doubling Time = ",average,"ms")
    
    
    # point identical Multiplication
    i=1
    num=500
    sum=0
    while i<=num :
   	r=random.randint(1,(pow(2,256)-1))
   	#print("\n Random Scalar =",r)
   	start = time.time() 
   	pimul=EccPoint.__imul__(point,r)
   	end = time.time()
	elapsed=(end-start)*1000
	sum+=elapsed 	
    	#print("\n After Scalar Multiplication X= ",pimul.xy[0],"\n and Y = ",pimul.xy[1] )
   	#print("\n Scalar Multiplication Time = ",elapsed, "ms")
    	i+=1
    average=sum/num
    print("\n Average scalar multiplication Time = ",average,"ms")
    ...
    while i<=num :
    	start = time.time()
    	piadd=EccPoint.__iadd__(point,point)
   	end = time.time()
        elapsed=(end-start)*1000
	sum+=elapsed 
    	#print("\n IdenticalPoint Addition X= ",piadd.xy[0],"\n and Y = ",piadd.xy[1] )
    	#print("\n Identical Addition Time = ",elapsed, "ms")
        i+=1
    average=sum/num
    print("\n Average Identical addition Time = ",average,"ms")
    ...
    sum=0
    while i<=num :
    	point2 = _curves['p256'].G.copy()
    	#print("\n point X=",point2.xy[0])
    	#print("\n point y=",point2.xy[1])
    	start = time.time()
    	padd=EccPoint.__add__(point,point2)
    	end = time.time()
        elapsed=(end-start)*1000
	sum+=elapsed 
    	#print("\n Two Different Point Addition X= ",padd.xy[0],"\n and Y = ",padd.xy[1] )
    	#print("\n Point Addition Time = ",(end-start)*1000, "ms")
        i+=1
    average=sum/num
    print("\n Average point addition Time = ",average,"ms")
...
#h= random.randint(1,(pow(2,256)-1))
k=random.randint(1,(pow(2,256)-1))
...
while i<=num :
    	start = time.time()
	r1,s1=EccKey._sign(key,z,k)
        end = time.time()
        elapsed=(end-start)*1000
	sum+=elapsed
	i+=1
average=sum/num
print("\n Average signature generation Time = ",average,"ms")

#ECDSA VERIFY	

f = open('myprivatekey.pem','rt')
key = ECC.import_key(f.read())
message = b'I give my permission to order #435510'
h = SHA256.new(message)
z=int(h.hexdigest(),16)
rs=[]
rs.append(r1)
...
while i<=num :
    	start = time.time()
	verifier=EccKey._verify(key,z,rs)
	end = time.time()
        elapsed=(end-start)*1000
	sum+=elapsed
	i+=1
...
average=sum/num
print("\n Average signature verification Time = ",average,"ms")
